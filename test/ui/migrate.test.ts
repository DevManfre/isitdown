...
test("seedDefaults registers every channel, disabled, referencing environment variables by name", async () => {
  const db = await freshDb();
  migrate(db);
  seedDefaults(db);
  ...
  const byId = new Map(rows.map((row) => [row.id, JSON.parse(row.config) as Record<string, string>]));
  assert.deepEqual(byId.get("telegram"), {
    botTokenEnv: "TELEGRAM_BOT_TOKEN",
    chatIdEnv: "TELEGRAM_CHAT_ID",
  });
  assert.deepEqual(byId.get("webhook"), { urlEnv: "WEBHOOK_URL", secretEnv: "WEBHOOK_SECRET" });
  assert.deepEqual(byId.get("discord"), { webhookUrlEnv: "DISCORD_WEBHOOK_URL" });
  assert.deepEqual(byId.get("slack"), { webhookUrlEnv: "SLACK_WEBHOOK_URL" });
  ...
  assert.deepEqual(byId.get("email"), {
    hostEnv: "SMTP_HOST",
    portEnv: "SMTP_PORT",
  ...
  // Matrix (roadmap 3.6), the on-call pair (3.7) and the Apprise bridge (3.9).
  assert.deepEqual(byId.get("matrix"), {
    homeserverUrlEnv: "MATRIX_HOMESERVER_URL",
    roomIdEnv: "MATRIX_ROOM_ID",
    accessTokenEnv: "MATRIX_ACCESS_TOKEN",
  });
  assert.deepEqual(byId.get("pagerduty"), {
    routingKeyEnv: "PAGERDUTY_ROUTING_KEY",
    regionEnv: "PAGERDUTY_REGION",
  });
  assert.deepEqual(byId.get("opsgenie"), {
    apiKeyEnv: "OPSGENIE_API_KEY",
    regionEnv: "OPSGENIE_REGION",
  });
  assert.deepEqual(byId.get("apprise"), {
    serverUrlEnv: "APPRISE_SERVER_URL",
    configKeyEnv: "APPRISE_CONFIG_KEY",
    urlsEnv: "APPRISE_URLS",
  });
  // webpush carries no reference at all: its VAPID pair is generated by the
  // server on first use (src/ui/vapidKeys.ts), so the channel has nothing for
  // the operator to configure beyond its switch.
  assert.deepEqual(byId.get("webpush"), {});
...
test("migrating from schema 6 drops the webpush channel's environment references", async () => {
  const db = await freshDb();
  migrate(db);
  // Roll back to how a running instance looked before this change: the webpush
  // row still pointing at the two VAPID variables the operator had to fill in.
  db.prepare("INSERT INTO channels (id, enabled, config) VALUES (?, 1, ?)").run(
    "webpush",
    JSON.stringify({ publicKeyEnv: "VAPID_PUBLIC_KEY", privateKeyEnv: "VAPID_PRIVATE_KEY" }),
  );
  db.prepare("INSERT INTO channels (id, enabled, config) VALUES (?, 1, ?)").run(
    "telegram",
    JSON.stringify({ botTokenEnv: "TELEGRAM_BOT_TOKEN", chatIdEnv: "TELEGRAM_CHAT_ID" }),
  );
  db.prepare(
    "INSERT INTO push_subscriptions (id, endpoint, p256dh, auth, label, created_at) VALUES (?, ?, ?, ?, ?, ?)",
  ).run("abc", "https://push.example/abc", "p", "a", "Chrome · Windows", "2026-08-31T00:00:00.000Z");
  db.exec("PRAGMA user_version = 6");

  migrate(db);

  const webpush = db.prepare("SELECT enabled, config FROM channels WHERE id = 'webpush'").get() as {
    enabled: number;
    config: string;
  };
  assert.deepEqual(JSON.parse(webpush.config), {});
  // An operator who had the channel on keeps it on: the migration removes a
  // configuration surface, it does not turn notifications off.
  assert.equal(webpush.enabled, 1);

  const devices = db.prepare("SELECT COUNT(*) AS n FROM push_subscriptions").get() as { n: number };
  // Subscriptions made against the old environment key pair cannot receive a
  // push signed with the generated one, so they go rather than linger as dead
  // rows in the device list.
  assert.equal(devices.n, 0);

  const telegram = db.prepare("SELECT config FROM channels WHERE id = 'telegram'").get() as { config: string };
  assert.deepEqual(JSON.parse(telegram.config), {
    botTokenEnv: "TELEGRAM_BOT_TOKEN",
    chatIdEnv: "TELEGRAM_CHAT_ID",
  });
  db.close();
