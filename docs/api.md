...
Unauthenticated by default: this is a single-operator dashboard bound to the
machine it runs on. Setting `API_TOKEN` turns on one read-only bearer token —
`GET` and `HEAD` from any host, everything else refused, `/health` and `/ready`
never gated. See [3.12](configuration.md#312-reaching-the-api-from-another-host).
...
| `GET` | `/sla` | Monthly targets and error budgets — roadmap 4.13. `{ month, providers: [{ providerId, target, uptime, budgetMinutes, spentMinutes, remainingMinutes, burnRate, projectedUptime, willMiss, elapsedMinutes, monthMinutes, measuredMinutes }] }`. One entry per provider that carries a target; providers without one are absent rather than reported at 100%. `uptime`, `burnRate` and `projectedUptime` are `null` for a month nothing measured — which is not 0%. See [3.13](configuration.md#313-sla-targets-and-error-budgets). |
| `GET` | `/trust?days=` | How closely each cross-checked status page tracked what a probe observed — roadmap 8.1. One card per probe that names a page in `crossChecks`: `{ days, windows, cards: [{ key, pair, counted, delay, coverage, never, afterRecovery, excluded, resolutionMinutes }] }`. Three axes, never one score. `days` accepts `30`, `90` or `365` and falls back to 90. A card whose window holds fewer than ten counted episodes carries `{ counted, floor }` and nothing else — the axes are absent rather than zeroed. A fleet with no probe answers `cards: []`, and the dashboard shows no section at all. |
| `GET` | `/trust/:key/episodes?days=` | Every disagreement behind one card, newest first, keyed `<probe>-><page>` or `<probe>-><page>#<component>`. Excluded episodes are included with their reason (`maintenance`, `fleet_blind`): they are how the card's excluded counts can be checked. `404` on a pair nothing cross-checks. |
...
| `GET` | `/notifications/preview?kind=` | What every configured channel would say about one invented transition — roadmap 14.1. `{ kind, provider, channels: [{ channel, enabled, locale, templated, text, parts: { heading, detail, url } }] }`. Nothing is sent and nothing is recorded: it renders from the same two pure functions every notifier builds its body with, so it cannot drift from a real send without the notifiers drifting too. `text` is the whole message, byte for byte, for a channel that posts text; it is `null` for one that assembles a structure of its own (a Slack block kit message, a Teams adaptive card, a PagerDuty event), and `parts` is what that channel builds from — deliberately not a mock-up of the structure, which would be a drawing of the message rather than the message, and which would be believed once it drifted. Disabled channels are included: setting a channel up is exactly when it is not enabled yet. `kind` is one of `status_change`, `incident_opened`, `incident_resolved`, `maintenance_started`, `monitoring_degraded`, `silent_outage`. |
| `GET` | `/reliability?days=` | Per-provider MTTR, MTBF and the longest outage, plus incidents by weekday and hour — roadmap 12.2 and 12.3, over rows the poller already wrote. `{ days, providers: [{ providerId, incidents, mttrMinutes, resolved, mtbfMinutes, longestOutageMinutes, downtimeMinutes, previousIncidents }], byWeekdayHour }`. `mttrMinutes` averages only the incidents that have actually been resolved — an open one is left out rather than measured to now, which would report a provider mid-outage as terrible and then improve it the moment the outage ended — and `resolved` is that average's own sample size. `mtbfMinutes` is `null` below two incidents: one has nothing to be between. Every figure is `null` rather than `0` when the window cannot answer. `byWeekdayHour` is `[weekday][hour]`, Monday first, in the operator's zone, counted by when an incident *started*. Providers are worst first, by downtime. |
| `GET` | `/annotations?days=&from=&to=&provider=` | The operator's own timeline markers over a window — roadmap 12.1. `{ annotations: [{ id, at, label, colour, providerId, createdAt }] }`, oldest first. The window is asked for exactly as `/history` asks for one. `provider` **widens** rather than narrows: it returns that provider's markers *and* the fleet-wide ones, because a deploy that broke one provider's page is the marker wanted on its chart. |
| `POST` | `/annotations` | Writes one marker, `{ at, label, colour, providerId? }`. `at` is an ISO timestamp, `label` 1 to 120 characters, `colour` one of `accent`, `warn`, `danger`, `neutral` — token names, never a hex value, so a marker stays legible in both themes. Omitting `providerId` means the whole fleet, which a deploy usually is; naming an unknown provider is a `404`. Answers the stored marker. Wiring a deploy pipeline to this endpoint is one line of CI, and it turns "was it our release or theirs?" into a glance. |
| `DELETE` | `/annotations/:id` | Removes one. `204` on success, `404` on an id that is not there — a stale id is never a silent no-op. |
...
| `GET` | `/export/trust.csv?days=` | One row per trust episode with the pair repeated on each: `probe_id,page_id,component_id,probe_down_at,probe_up_at,page_admitted_at,outcome,excluded,delay_minutes,observed_minutes,admitted_minutes,resolution_minutes`. |
| `GET` | `/export/trust.json?days=` | The cards with their episodes, as `{ generatedAt, days, vantagePoint, pairs }`. Carries what it excluded and why, the error bar the poll cadence imposes, and the fact that all of it was seen from one container on one network — this is the export that gets quoted at a provider, and a median with no error bar reads like a measurement rather than an observation. |
| `GET` | `/export/monthly.md?month=` | One calendar month written up as Markdown (roadmap 4.7): uptime, downtime, incident count and worst day per provider, the fleet figure, and every incident that was open at any point in the month. `month` is `YYYY-MM` and defaults to the current one; a month still running is reported as far as today and says so in the document rather than being refused. A provider with no samples that month reads `—`, never `0%`. |
| `GET` | `/export/gatus.yaml` | The fleet as a Gatus `config.yaml` endpoint list (roadmap 17.4). Only `http`, `tcp` and `dns` probes have a Gatus equivalent — a status-page adapter is a document to read, not a thing Gatus can check itself, so it is skipped and counted in the file's own header rather than turned into a check against its status page. A disabled provider is included with `enabled: false`; a removed (soft-deleted) one is not included at all. A `dns` probe left on the system resolver is skipped too: Gatus needs an explicit server address to query. |
...
| `PATCH` | `/config/channels/:id` | Enable/disable, set variable names, and set the channel's own `locale` ([3.20](configuration.md#311-per-channel-locale-and-template)) and `template` ([3.15](configuration.md#311-per-channel-locale-and-template)). **Refuses** a literal secret. An empty string clears the locale or the template back to the default; a template naming an unknown token is a `400` that says which. |
...
| `GET` `PATCH` | `/api/preferences` | `{ theme, uiLocale, notificationLocale, mapView, density, timeZone }`. `density` is `comfortable` or `compact` — how tightly the fleet lists pack (roadmap 13.1), stored rather than derived from the viewport because it is about how many providers there are. `timeZone` is `auto` — this browser's own — or an IANA name; anything the runtime cannot format a date in is refused. |
...
| `POST` | `/push/:providerId?token=` | A provider's own webhook — roadmap 2.10. Registered as the subscriber URL on a Statuspage page; the body is a trigger, not a reading, so the provider is then read through its adapter and goes through the same diff engine a scheduled cycle does. Needs `PUSH_TOKEN` set and matched, or `404`/`401`. Posts within ten seconds of one another are `202 coalesced`. The one route the read-only API token does not gate. See [3.15](configuration.md#315-provider-push-instead-of-poll). |
