...
| `GET` | `/trust?days=` | Quanto da vicino ogni status page sotto verifica incrociata ha seguito ciò che una sonda ha osservato — roadmap 8.1. Una scheda per ogni sonda che nomina una pagina in `crossChecks`: `{ days, windows, cards: [{ key, pair, counted, delay, coverage, never, afterRecovery, excluded, resolutionMinutes }] }`. Tre assi, mai un punteggio unico. `days` accetta `30`, `90` o `365` e ricade su 90. Una scheda la cui finestra contiene meno di dieci episodi contati porta `{ counted, floor }` e nient'altro — gli assi sono assenti, non azzerati. Una flotta senza sonde risponde `cards: []`, e il dashboard non mostra alcuna sezione. |
| `GET` | `/trust/:key/episodes?days=` | Ogni disaccordo dietro una scheda, dal più recente, con chiave `<sonda>-><pagina>` oppure `<sonda>-><pagina>#<componente>`. Gli episodi esclusi sono inclusi con il loro motivo (`maintenance`, `fleet_blind`): sono il modo di verificare i conteggi di esclusione della scheda. `404` su una coppia che nessuno verifica in modo incrociato. |
...
| `GET` | `/notifications/preview?kind=` | Cosa direbbe ogni canale configurato su una transizione finta — roadmap 14.1. `{ kind, provider, channels: [{ channel, enabled, locale, templated, text, parts: { heading, detail, url } }] }`. Non invia e non registra nulla: rende dalle stesse due funzioni pure con cui ogni notifier costruisce il proprio corpo, quindi non può divergere da un invio reale senza che divergano anche i notifier. `text` è il messaggio intero, byte per byte, per un canale che posta testo; è `null` per uno che assembla una struttura propria (un messaggio block kit di Slack, una adaptive card di Teams, un evento PagerDuty), e `parts` è ciò da cui quel canale parte — deliberatamente non un mock-up della struttura, che sarebbe il disegno del messaggio e non il messaggio, e a cui si crederebbe una volta divergente. I canali disattivi sono inclusi: configurare un canale è esattamente il momento in cui non è ancora attivo. `kind` è uno tra `status_change`, `incident_opened`, `incident_resolved`, `maintenance_started`, `monitoring_degraded`, `silent_outage`. |
| `GET` | `/reliability?days=` | MTTR, MTBF e disservizio più lungo per provider, più gli incidenti per giorno della settimana e ora — roadmap 12.2 e 12.3, su righe che il poller ha già scritto. `{ days, providers: [{ providerId, incidents, mttrMinutes, resolved, mtbfMinutes, longestOutageMinutes, downtimeMinutes, previousIncidents }], byWeekdayHour }`. `mttrMinutes` media solo gli incidenti davvero risolti — uno aperto resta fuori invece di essere misurato fino ad adesso, cosa che riporterebbe come pessimo un provider in pieno disservizio per poi migliorarne la cifra appena il disservizio finisce — e `resolved` è la numerosità di quella media. `mtbfMinutes` è `null` sotto i due incidenti: uno solo non ha nulla in mezzo a cui stare. Ogni cifra è `null` e non `0` quando la finestra non può rispondere. `byWeekdayHour` è `[giorno][ora]`, lunedì per primo, nel fuso dell'operatore, contato per quando un incidente è *iniziato*. I provider sono dal peggiore, per minuti di disservizio. |
| `GET` | `/annotations?days=&from=&to=&provider=` | I marcatori dell'operatore sulla timeline, su una finestra — roadmap 12.1. `{ annotations: [{ id, at, label, colour, providerId, createdAt }] }`, dal più vecchio. La finestra si chiede esattamente come la chiede `/history`. `provider` **allarga** invece di restringere: restituisce i marcatori di quel provider *e* quelli di flotta, perché un deploy che ha rotto la pagina di un provider è proprio il marcatore che si vuole sul suo grafico. |
| `POST` | `/annotations` | Scrive un marcatore, `{ at, label, colour, providerId? }`. `at` è un timestamp ISO, `label` da 1 a 120 caratteri, `colour` uno tra `accent`, `warn`, `danger`, `neutral` — nomi di token, mai un valore esadecimale, così un marcatore resta leggibile in entrambi i temi. Omettere `providerId` significa l'intera flotta, che è ciò che un deploy di solito è; indicare un provider sconosciuto è un `404`. Risponde con il marcatore salvato. Collegare qui la pipeline di deploy è una riga di CI, e trasforma "è colpa della nostra release o della loro?" in un'occhiata. |
| `DELETE` | `/annotations/:id` | Ne rimuove uno. `204` se riuscito, `404` su un id che non c'è — un id vecchio non è mai un no-op silenzioso. |
...
| `GET` | `/export/trust.csv?days=` | Una riga per episodio di fiducia, con la coppia ripetuta su ognuna: `probe_id,page_id,component_id,probe_down_at,probe_up_at,page_admitted_at,outcome,excluded,delay_minutes,observed_minutes,admitted_minutes,resolution_minutes`. |
| `GET` | `/export/trust.json?days=` | Le schede con i loro episodi, come `{ generatedAt, days, vantagePoint, pairs }`. Porta con sé cosa ha escluso e perché, la barra d'errore imposta dalla cadenza di poll, e il fatto che tutto è stato visto da un solo container su una sola rete — questo è l'export che finisce citato contro un provider, e una mediana senza barra d'errore si legge come una misura invece che come un'osservazione. |
...
| `GET` | `/export/gatus.yaml` | La flotta come lista di endpoint Gatus in un `config.yaml` (roadmap 17.4). Solo le sonde `http`, `tcp` e `dns` hanno un equivalente in Gatus — un adapter basato su status page è un documento da leggere, non una cosa che Gatus può verificare da sé, quindi viene saltato e conteggiato nell'header del file invece di diventare un controllo contro la sua status page. Un provider disabilitato è incluso con `enabled: false`; uno rimosso (cancellazione soft) non è incluso affatto. Anche una sonda `dns` lasciata sul resolver di sistema viene saltata: Gatus richiede un indirizzo di server esplicito da interrogare. |
...
| `GET` `PATCH` | `/api/preferences` | `{ theme, uiLocale, notificationLocale, mapView, density, timeZone }`. `density` è `comfortable` o `compact` — quanto fitte stanno le liste della flotta (roadmap 13.1), salvata invece che dedotta dal viewport perché riguarda quanti provider ci sono. `timeZone` è `auto` — il fuso di questo browser — oppure un nome IANA; qualunque valore in cui il runtime non sappia formattare una data viene rifiutato. |
