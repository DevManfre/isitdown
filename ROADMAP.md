...
| 1.8 ◐ | **Direct HTTP probe** ("is *my* thing up") | L ⚠️ | Identity settled the narrow way: not a second concept beside a provider, but the one adapter whose subject is a service rather than a page about one — so groups, routing, history, badges and exports came for free rather than being rebuilt. Shipped as `adapter: http`: `method` (GET/HEAD only, since the poller retries), `path`, `expectStatus` as codes and inclusive ranges, `expectBody` / `absentBody`, `slowMs` reading `degraded`, `followRedirects`, and `header.<Name>` resolving `${VAR}` from the environment. It inverts the contract every other adapter holds — a non-2xx and an unreachable host are *readings* here, not failed reads, so the diff engine sees a status change where the poller would have seen a retry — which the contract kit now states as an explicit flag rather than an exemption. Only the configuration throws, and `check` reads it offline. Left: TLS-expiry warning, which is 1.9's territory rather than a missing option here (it needs a second connection and cert fixtures to test honestly), *why* it is down surfaced in diagnostics (a reading carries a severity and nothing else), and the guard telling a network failure of our own apart from every target being down at once. No allowlist on what it may reach — deliberate for a dashboard on `127.0.0.1`, and a precondition 4.15 and 5.1 have to settle first. |
